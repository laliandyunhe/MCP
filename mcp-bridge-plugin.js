(function () {
  ...
  var MCPClient = (function () {
    ...
    MCPConnection.prototype._nextId = function () {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (line.indexOf("data:") === 0) {
            ...
            try {
              ...
              if (e.message && e.message.indexOf("MCP错误") === 0) throw e;
            }
          }
        }
      }
    ...
    MCPConnection.prototype._rpcHTTP = async function (method, params) {
      ...
      if (!resp.ok) {
        var t = await resp.text().catch(function () {
          return "";
        });
      ...
      if (json.error)
        throw new Error(
          "MCP错误: " + (json.error.message || JSON.stringify(json.error)),
        );
      return json.result;
    };
    ...
    MCPConnection.prototype._request = function (method, params) {
      return this.transport === "sse"
        ? this._rpcSSE(method, params)
        : this._rpcHTTP(method, params);
    };
    ...
    MCPConnection.prototype.connect = async function () {
          ...
          es.onmessage = function (evt) {
            try {
              ...
              if (msg.id && self._pendingRequests[msg.id]) {
                var p = self._pendingRequests[msg.id];
                delete self._pendingRequests[msg.id];
                msg.error
      ...
      await this._request("initialize", {
        ...
        capabilities: {},
        clientInfo: { name: "roche-mcp-bridge", version: "1.0.0" },
      });
    ...
    MCPConnection.prototype.describeTools = function () {
      ...
      return this.tools
        .map(function (t) {
          ...
          var paramDesc = Object.keys(props)
            .map(function (k) {
              var p = props[k];
  ...
  function renderTopbar(isSettings) {
    ...
    el.style.cssText =
      "display:flex;align-items:center;padding:12px 8px 8px;background:#f2f2f7;flex-shrink:0;gap:4px;border-bottom:0.5px solid #e8e8ed;";
      ...
      if (isSettings) {
        ...
        closeBtn.onclick = function () {
          ...
          if (
            state.roche &&
            state.roche.ui &&
            typeof state.roche.ui.closeApp === "function"
          ) {
  ...
  function renderModebar() {
    ...
    [
      ...
      btn.className =
        "mcp-mode-btn" + (state.mode === pair[0] ? " active" : "");
  ...
  function renderServerChips() {
    ...
    state.configs.forEach(function (cfg) {
      var conn = state.servers.find(function (s) {
        return s.id === cfg.id;
      });
      ...
      if (state.connecting[cfg.id]) {
        ...
        connectBtn.style.cssText =
          "font-size:11px;padding:3px 8px;background:#6366f1;color:#fff;border:none;border-radius:6px;cursor:pointer;";
  ...
  function renderToolPanel() {
    ...
    if (!connectedServers.length) {
      ...
      empty.innerHTML =
        I.plug +
        '<div class="mcp-empty-title">暂无可用工具</div><div class="mcp-empty-desc">连接 MCP 服务器后，工具列表将显示在此处</div>';
      el.appendChild(empty);
      return el;
    }

    connectedServers.forEach(function (server) {
      var serverCfg = state.configs.find(function (c) {
        return c.id === server.id;
      });
      var disabledTools = (serverCfg && serverCfg.disabledTools) || [];
      var enabledTools = server.tools.filter(function (t) {
        return disabledTools.indexOf(t.name) === -1;
      });
  ...
  function renderAutoHint() {
    ...
    el.innerHTML =
      I.zap +
      '<div class="mcp-auto-hint-title">AI 自主模式已启用</div><div class="mcp-auto-hint-desc">每次发送消息时，AI 会自动判断是否需要调用工具。工具调用过程和结果将以气泡形式显示在对话中。</div>';
    return el;
  }
  ...
  function renderSettings() {
    ...
    state.configs.forEach(function (cfg) {
      ...
      info.appendChild(iurl);
      info.appendChild(imeta);
      ...
      delBtn.onclick = function () {
        ...
        var conn2 = state.servers.find(function (s) {
          return s.id === cfg.id;
        });
        if (conn2) {
          conn2.disconnect();
    ...
    showProcessRow.innerHTML =
      '<div class="mcp-toggle-info"><div class="mcp-toggle-title">显示工具调用过程</div><div class="mcp-toggle-desc">在对话中以气泡显示工具名称、参数和返回结果</div></div>';
  ...
  function renderServerForm() {
    ...
    var currentHeaders = Array.isArray(cfg.headers)
      ? cfg.headers.slice()
      : Object.keys(cfg.headers || {}).map(function (k) {
          return { key: k, val: cfg.headers[k] };
        });
    ...
    [
      ...
      tab.onclick = function () {
        ...
        tabbar.querySelectorAll(".mcp-tab").forEach(function (t) {
    ...
    var enableSwitch = makeIosSwitch(currentEnabled, function (v) {
      currentEnabled = v;
    });
    ...
    [
      ...
      var btn = document.createElement("button");
      btn.className =
        "mcp-segment-btn" + (currentTransport === pair[0] ? " active" : "");
      btn.textContent = pair[1];
      btn.onclick = function () {
        currentTransport = pair[0];
        segment.querySelectorAll(".mcp-segment-btn").forEach(function (b) {
    ...
    function renderHeaderItems() {
      ...
      currentHeaders.forEach(function (h, idx) {
        ...
        kInput.oninput = function () {
          currentHeaders[idx].key = kInput.value;
        };
        ...
        vInput.oninput = function () {
          currentHeaders[idx].val = vInput.value;
        };
        var delBtn = document.createElement("button");
    ...
    if (!conn || !conn.initialized) {
      ...
      emptyTools.innerHTML =
        I.plug +
        '<div class="mcp-empty-title">未连接</div><div class="mcp-empty-desc">保存并连接服务器后，可在此处启用或禁用各工具</div>';
      ...
      noTools.innerHTML =
        I.plug +
        '<div class="mcp-empty-title">无可用工具</div><div class="mcp-empty-desc">该服务器未返回任何工具</div>';
      toolsScroll.appendChild(noTools);
    } else {
      // 从 configs 中读取该服务器的禁用工具列表（用本地副本操作，保存时才写回）
      var existingCfgForTools = state.configs.find(function (c) {
        return c.id === cfg.id;
      });
      var disabledTools = (
        (existingCfgForTools && existingCfgForTools.disabledTools) ||
        []
      ).slice();
      ...
      toolsSectionHint.textContent =
        "禁用的工具不会出现在手动调用面板和 AI 自主调用中";
      ...
      conn.tools.forEach(function (tool, idx) {
        ...
        if (tool.inputSchema && tool.inputSchema.properties) {
          ...
          if (keys.length) {
            ...
            paramPreview.style.cssText =
              "font-size:11px;color:#c7c7cc;margin-top:3px;";
        ...
        (function (toolName) {
          var sw = makeIosSwitch(isEnabled, function (enabled) {
            if (enabled) {
              ...
              if (disabledTools.indexOf(toolName) === -1)
      ...
      toolsSaveBtn.onclick = function () {
        var targetCfg = state.configs.find(function (c) {
          return c.id === cfg.id;
        });
        if (targetCfg) {
    ...
    function doSave() {
      ...
      currentHeaders.forEach(function (h) {
        if (h.key && h.key.trim())
          headersObj[h.key.trim()] = (h.val || "").trim();
      });

      if (cfg.id) {
        var existing = state.configs.find(function (c) {
          return c.id === cfg.id;
        });
        if (existing) {
          existing.name = name;
          existing.url = url;
          existing.transport = currentTransport;
          existing.headers = headersObj;
        ...
        var newId =
          String(Date.now()) + Math.random().toString(36).substring(2, 6);
        var newCfg = {
          id: newId,
          name: name,
          url: url,
      ...
      // 如果启用，自动连接
      var savedCfg = state.configs.find(function (c) {
        return c.name === name && c.url === url;
      });
  ...
  function makeSwitch(initial, onChange) {
    return makeIosSwitch(initial, onChange);
  }
  ...
  function injectToolResult(serverName, toolName, args, result, isError) {
    ...
    bubble.className =
      "mcp-result-bubble" + (isError ? " mcp-result-bubble-error" : "");
    ...
    // 找聊天容器注入
    var chatLog =
      document.querySelector(".drip-chat-log") ||
  ...
  async function handleNonStream(resp, opts) {
    ...
    if (callSpec && callSpec.server && callSpec.tool) {
      ...
      if (targetServer) {
        ...
        var isError = false;
        try {
          rawResult = await targetServer.callTool(callSpec.tool, callSpec.args || {});
          var content = rawResult && rawResult.content;
          toolResultText = Array.isArray(content)
            ? content.map(function (c) { return c.text || JSON.stringify(c); }).join("\n")
            : typeof rawResult === "object" ? JSON.stringify(rawResult, null, 2) : String(rawResult || "（无返回内容）");
        } catch (e) {
          isError = true;
          rawResult = e.message;
        ...
        // 核心修复：后台自动执行时调用气泡渲染！
        injectToolResult(targetServer.name, callSpec.tool, callSpec.args, rawResult, isError);

        var r2Messages = opts.messages.concat([
          { role: "assistant", content: fullText },
          {
            role: "user",
            content: "工具调取完成，这是返回的数据（请根据此数据为我解答，切勿再次输出 mcp_call 指令）：\n```\n" + toolResultText + "\n```"
          }
        ]);
      ...
      if (targetServer) {
        ...
        try {
          ...
          if (Array.isArray(content)) {
            ...
            toolResultText = typeof rawResult === "object" ? JSON.stringify(rawResult, null, 2) : String(rawResult || "（无返回内容）");
          }
        } catch (e) {
          isError = true;
          rawResult = e.message;
  ...
  // 提取 JSON 工具调用参数 (使用 XML 优先 + 智能容错)
  function extractToolCall(text) {
    // 1. 优先匹配稳定的 XML 格式
    var xmlMatch = text.match(/<mcp_call>([\s\S]*?)<\/mcp_call>/i);
    if (xmlMatch) {
      try {
        var parsed = JSON.parse(xmlMatch[1].trim());
        if (parsed.server && parsed.tool) return parsed;
      } catch (e) {}
    }

    // 2. 兼容 AI 习惯性包裹 Markdown 的情况
    var jsonMatch = text.match(/```json\s*(\{[\s\S]*?\})\s*```/i);
    if (!jsonMatch) {
      jsonMatch = text.match(/```\s*(\{[\s\S]*?\})\s*```/i);
    }
    if (jsonMatch) {
      try {
    ...
    // 3. 极其贪婪的兜底（仅提取包裹对象的 JSON，修复原版贪婪全文本崩溃问题）
    var rawMatch = text.match(/\{[\s\S]*?"server"[\s\S]*?"tool"[\s\S]*?\}/i);
    if (rawMatch) {
      try {
        var parsed = JSON.parse(rawMatch[0].trim());
        if (parsed.mcp_call) return parsed.mcp_call;
        if (parsed.server && parsed.tool) return parsed;
      } catch (e) {}
    }
  ...
  async function* wrapStream(origStream, opts) {
    ...
    if (callSpec && callSpec.server && callSpec.tool) {
      ...
      if (targetServer) {
        ...
        var toolResultText = "";
        try {
          var res = await targetServer.callTool(
            callSpec.tool,
            callSpec.args || {},
          );
        ...
        var r2Opts = Object.assign({}, opts, { messages: r2Messages });
        var r2Stream = await state.roche.ai.__mcpOrigChat.call(
          state.roche.ai,
          r2Opts,
        );
        ...
        if (r2Stream && typeof r2Stream[Symbol.asyncIterator] === "function") {
          ...
          var txt =
            typeof r2Stream === "string"
              ? r2Stream
              : r2Stream.text || r2Stream.content || JSON.stringify(r2Stream);
          yield isStringStream ? txt : { text: txt, content: txt };
        }
      }
    }
  ...
  function startAutoMode() {
    ...
    // 核心修复：防止切换页面导致多重套娃代理
    if (state.roche.ai.chat.isMcpWrapper) return;
    ...
    var wrapper = async function (opts) {
      ...
      // 核心修复：强化 Prompt，使用 XML 标签规范模型输出行为，显著提升触发率
      var toolSystemPrompt = [
        "【系统指令：MCP 外部工具】",
        "你可以自主判断是否需要调用以下工具来获取实时信息或执行操作以辅助回答：",
        toolsDesc,
        "若决定调用工具，请务必且仅输出以下XML标签，在标签内提供JSON格式的参数（一次仅调用一个工具）：",
        "<mcp_call>",
        '{"server": "服务器名", "tool": "工具名", "args": {"参数1": "值1"}}',
        "</mcp_call>",
        "如果可以直接回答，请正常回复，绝对不要输出 <mcp_call> 标签。"
      ].join("\n");
    ...
    // 标记这是我们的 wrapper
    wrapper.isMcpWrapper = true;
    state.roche.ai.chat = wrapper;
  }
    ...
    apps: [
      {
        ...
        // 在 window.RochePlugin.register 的 unmount 中：
        unmount: function (container) {
          // 核心修复：不再移除 styleEl！保留 CSS，防止后台运行时注入到对话里的气泡变成透明乱码
