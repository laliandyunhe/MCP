(function () {
  ...
  /* ============================================================
     持久化（修复：改用异步 Promise 接口）
     ============================================================ */
  var STORAGE_KEY = "mcp-bridge-v1";

  async function loadConfig(roche) {
    try {
      var raw = await roche.storage.get(STORAGE_KEY);
      if (!raw) return;
  ...
  async function saveConfig(roche) {
    try {
      ...
      await roche.storage.set(STORAGE_KEY, data);
    } catch (e) {}
  }
  ...
  function renderTopbar(isSettings) {
    ...
    if (isSettings) {
      ...
      // 主界面：完全由宿主控制容器生命周期，这里不提供单独的关闭/返回键，只用空白块占位保持标题居中
      var placeholderLeft = document.createElement("div");
      placeholderLeft.style.width = "52px";
      el.appendChild(placeholderLeft);
    }
    ...
    if (!isSettings) {
      ...
      // 占位保持对称
      var placeholderRight = document.createElement("div");
      placeholderRight.style.width = "52px";
      el.appendChild(placeholderRight);
    }
  ...
  function renderSettings() {
    ...
    var sw = makeSwitch(state.showProcess, async function (v) {
      state.showProcess = v;
      await saveConfig(state.roche);
    });
  ...
  function renderServerForm() {
    ...
    var currentEnabled = cfg.enabled !== false; 
    var currentTab = state._formTab || "basic"; 
    ...
    if (!conn || !conn.initialized) {
      ...
      toolsSaveBtn.onclick = async function () {
    ...
    async function doSave() {
      ...
      if (cfg.id) {
        ...
        if (existing) {
          // 精准比较核心网络连线参数
          var isUrlChanged = existing.url !== url;
          var isTransportChanged = existing.transport !== currentTransport;
          var isHeadersChanged = JSON.stringify(existing.headers) !== JSON.stringify(headersObj);
          var isEnabledChanged = existing.enabled !== currentEnabled;

          // 只有网络核心数据变了，或从禁用变为启用，才做断连/重连刷新操作
          if (isUrlChanged || isTransportChanged || isHeadersChanged || isEnabledChanged) {
            ...
            delete state.errors[cfg.id];
            shouldReconnect = currentEnabled; // 只有在依然启用的情况下才拉起重新连接
          }
        ...
        shouldReconnect = currentEnabled;
      }

      await saveConfig(state.roche);
      var savedId = cfg.id || state.configs[state.configs.length - 1].id;
      state.editingServer = undefined;
      state._formTab = undefined;

      if (shouldReconnect) {
        var savedCfg = state.configs.find(function (c) { return c.id === savedId; });
        if (savedCfg) connectServer(savedCfg);
      }
  ...
  function startAutoMode() {
    ...
    state.roche.ai.chat = async function (opts) {
      ...
      if (!toolCallMatch) return firstResp; 
    ...
    apps: [
      {
        ...
        mount: async function (container, roche) {
          ...
          // 修复：在这里等待配置异步加载完毕后再执行下一步
          await loadConfig(roche);

          // 自动连接上次已配置且处于“开启”状态的服务器
          state.configs.forEach(function (cfg) { 
