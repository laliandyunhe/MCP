  ...
  function renderTopbar(isSettings) {
    ...
    if (isSettings) {
      ...
      closeBtn.onclick = function () {
        // 尝试调用宿主平台的关闭/隐藏插件方法
        if (state.roche && state.roche.ui && typeof state.roche.ui.closePlugin === "function") {
          state.roche.ui.closePlugin();
        } else if (state.roche && typeof state.roche.closePlugin === "function") {
          state.roche.closePlugin();
        } else {
          // 回退：触发浏览器历史后退（部分宿主用路由管理插件视图）
          window.history.back();
        }
      };
