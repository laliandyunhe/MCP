(function () {
  ...
  function renderServerForm() {
    ...
    if (!conn || !conn.initialized) {
      ...
      emptyTools.innerHTML = I.plug + '<div class="mcp-empty-title">未连接</div><div class="mcp-empty-desc">保存后连接服务器，工具列表将显示在此处</div>';
      ...
      toolsSection.className = "mcp-section";
      conn.tools.forEach(function (tool) {
        ...
        info.appendChild(tName); info.appendChild(tDesc);
        var chev = document.createElement("span");
        chev.className = "mcp-tool-chevron";
        chev.innerHTML = I.chevDown;
        header.appendChild(info); header.appendChild(chev);
        card.appendChild(header);
        toolsSection.appendChild(card);
      });
      toolsScroll.appendChild(toolsSection);
    }
